...
StringPiece next(StringPiece data) {
  const char *sp =
      static_cast<const char *>(::memchr(data.data(), ' ', data.size()));
  if (sp) {
...
void ProcessInspector::registerCommands(Inspector *ins) {
  ...
  // ins->add("proc", "opened_files", ProcessInspector::openedFiles, "count
  // /proc/self/fd");
  ins->add("proc", "threads", ProcessInspector::threads,
           "list /proc/self/task");
}

string ProcessInspector::overview(HttpRequest::Method,
...
string ProcessInspector::threads(HttpRequest::Method,
                                 const Inspector::ArgList &) {
